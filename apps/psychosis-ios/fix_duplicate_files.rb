...
files_to_check.each do |filename|
  ...
  if matching_refs.length > 1
    ...
    # Keep the one with the full PsychosisApp/ path, remove others
    full_path_ref = matching_refs.find { |r| r.path&.start_with?('PsychosisApp/') }
    short_path_refs = matching_refs.reject { |r| r.path&.start_with?('PsychosisApp/') }
