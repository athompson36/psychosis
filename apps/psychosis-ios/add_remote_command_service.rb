...
# Script to add RemoteCommandService to Xcode project
require 'xcodeproj'
...
file_path = 'Core/Services/RemoteCommandService.swift'
full_path = File.join('PsychosisApp', file_path)
actual_path = File.join(__dir__, 'PsychosisApp', file_path)

unless File.exist?(actual_path)
  puts "Error: File not found: #{actual_path}"
  exit 1
end
