...
puts "Correct base path: ../../apps/psychosis-ios/PsychosisApp\n"
...
files_to_fix.each do |filename|
  ...
  matching_refs.each do |ref|
    ...
    # Check if path is wrong (starts with PsychosisApp/ but should be ../apps/psychosis-ios/PsychosisApp/)
    if current_path.start_with?('PsychosisApp/')
      # Calculate correct relative path from project location
      # Project is at: Psychosis/Psychosis.xcodeproj
      # Files are at: apps/psychosis-ios/PsychosisApp/...
      # Relative path from Psychosis/: ../apps/psychosis-ios/PsychosisApp/...
      
      correct_path = "../apps/psychosis-ios/#{current_path}"
    ...
    elsif current_path.start_with?('../apps/psychosis-ios/PsychosisApp/')
      puts "✅ Already correct: #{current_path}"
    else
      # Try to find the correct path
      correct_path = "../apps/psychosis-ios/PsychosisApp/#{current_path.split('/').last}"
      project_dir = File.dirname(project_path)
      full_correct_path = File.expand_path(correct_path, project_dir)
