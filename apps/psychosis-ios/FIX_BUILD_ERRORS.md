# Fix Build Errors: PsychosisApp.swift and Duplicate Files

## Current Issues

1. ❌ **`PsychosisApp.swift` reference exists but file is deleted** - causing "Build input file cannot be found"
2. ⚠️ **Duplicate file references** - files added multiple times causing "Skipping duplicate" warnings

## Step-by-Step Fix

### Step 1: Remove PsychosisApp.swift from Build Phases
...
6. **Find `PsychosisApp.swift`** in the list (it will show the full path)
7. **Select `PsychosisApp.swift`** in the list
...
**Important**: `PsychosisApp.swift` should **NOT** appear anywhere.
...
1. **Build Phases Check**:
   ...
   - Verify:
     ...
     - ❌ `PsychosisApp.swift` does NOT appear
...
### If PsychosisApp.swift still appears in Build Phases:
